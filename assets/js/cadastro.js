...
form?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const confirmacaoPendente = !authData.session;
    if (authData.session) await sb.auth.signOut({ scope: 'local' }).catch(() => {});
    const params = new URLSearchParams({
      cadastro: 'sucesso',
      confirmacao: confirmacaoPendente ? 'pendente' : 'dispensada',
      email: payloadAluno.email
    });
