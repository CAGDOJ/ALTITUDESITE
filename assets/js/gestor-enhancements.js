...
(() => {
  ...
  window.abrirAba = function abrirAbaMelhorada(id) {
    ...
    $('#btnMenuGestor')?.setAttribute('aria-expanded', 'false');
    ...
    try {
      ...
      if (list) list.innerHTML = data.length ? data.map(c => `
        <button type="button" onclick="abrirAba('cursos')">
          <img src="${escapeHtml(c.capa_url || '/assets/img/LOGO.png')}" alt="">
  ...
  async function alternarPublicacaoCurso(courseId, button, options = {}) {
    ...
    try {
      ...
      toast(next
        ? 'Curso publicado. Ele já pode aparecer no catálogo de Cursos.'
  ...
  async function importarCursoJson() {
    ...
    try {
      ...
      const { data: course, error: courseError } = await sb.from('cursos').insert({
        ...
        categoria: (window.altitudeNormalizarArea?.(payload.categoria || 'Formação') || String(payload.categoria || 'Formação').trim()),
  ...
  async function wire() {
    ...
    $('#btnMenuGestor')?.addEventListener('click', () => {
      ...
      $('#btnMenuGestor')?.setAttribute('aria-expanded', String(opened));
    });
    ...
    $('#btnSairGestor')?.addEventListener('click', async () => { await sb.auth.signOut(); location.href = '/login-gestor/'; });
