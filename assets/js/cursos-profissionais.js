(function () {
    ...
    return `
      <article class="public-course-card" data-id="${Number(curso.id)}" data-curso-tipo="${escapeHTML(String(curso.tipo_curso || 'PROFISSIONAL').toUpperCase())}">
        <div class="public-course-cover">
          <img src="${escapeHTML(capa(curso.capa_url))}" alt="Capa do curso ${escapeHTML(curso.titulo)}" loading="lazy" onerror="this.src='/assets/img/LOGO.png'">
          <img class="course-brand-mark" src="/assets/img/LOGO.png" alt="" aria-hidden="true">
  ...
  async function inscrever(cursoId) {
    ...
    if (!data?.session) {
      ...
      window.location.href = `/login/?curso=${id}`;
      return;
    }
    ...
    try {
      ...
      setTimeout(() => { window.location.href = `/portaldoaluno/?curso=${id}`; }, 700);
    } catch (error) {
      console.error(error);
  ...
  function wire() {
    $("filtrosTipoCurso")?.addEventListener("click", (event) => {
      ...
      if (title) title.textContent = state.tipo === "TECNICO" ? "Cursos Técnicos" : "Cursos";
      renderCategorias();
      renderCargas();
      renderCursos();
    });
