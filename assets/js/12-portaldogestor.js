...
function carregarAlunos(){
  if($('#alnBusca')){
    ...
    $('#alnNovo').addEventListener('click', async ()=> {
      const url = new URL('/cadastro/', window.location.href).href;
...
(function () {
  ...
  const normalizarArea = (valor) => {
    const texto = String(valor || '').trim().replace(/\s+/g, ' ');
    if (!texto) return '';
    const minusculas = new Set(['de','da','do','das','dos','e','em','para']);
    return texto.toLocaleLowerCase('pt-BR').split(' ').map((parte, indice) => {
      if (indice > 0 && minusculas.has(parte)) return parte;
      return parte.charAt(0).toLocaleUpperCase('pt-BR') + parte.slice(1);
    }).join(' ');
  };
  const areaKey = (valor) => String(valor || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().toLocaleLowerCase('pt-BR');
  const fmtArea = (valor) => normalizarArea(valor) || 'Sem área';
  window.altitudeNormalizarArea = normalizarArea;
  function areasCustomizadas() {
    try { return JSON.parse(localStorage.getItem(AREAS_STORAGE_KEY) || '[]').filter(Boolean); }
    catch (_) { return []; }
  }
  function salvarAreaCustomizada(area) {
    const nova = normalizarArea(area);
    if (!nova) return '';
    const lista = areasCustomizadas();
    if (!lista.some((item) => areaKey(item) === areaKey(nova))) {
      lista.push(nova);
      localStorage.setItem(AREAS_STORAGE_KEY, JSON.stringify(lista));
    }
    return nova;
  }
  ...
  const thumb   = (url) => url || '/assets/img/LOGO.png';
  ...
  async function carregarAreasCadastradas() {
    try {
      const { data, error } = await sb.from('areas_cursos_v36').select('nome').order('nome');
      if (error) throw error;
      GC.areasDb = (data || []).map((item) => normalizarArea(item.nome)).filter(Boolean);
      renderAreasSelects();
    } catch (error) {
      // Compatibilidade enquanto a migration V36 ainda não foi aplicada.
      console.warn('Áreas V36:', error.message);
      GC.areasDb = [];
    }
  }

  function renderAreasSelects(preferida = '') {
    const areas = areasDisponiveis();
    const filtro = $('#curFiltroArea');
    if (filtro) {
      const atual = preferida || filtro.value || 'TODAS';
      filtro.innerHTML = ['TODAS', ...areas]
        .map((a) => a === 'TODAS' ? '<option value="TODAS">Todas as áreas</option>' : `<option value="${escapeHTML(a)}">${escapeHTML(a)}</option>`)
        .join('');
      const compativel = [...filtro.options].find((opt) => areaKey(opt.value) === areaKey(atual));
      filtro.value = compativel?.value || 'TODAS';
    }

    ['#fCursoArea', '#iaCategoria'].forEach((selector) => {
      const select = $(selector);
      if (!select) return;
      const atual = preferida || select.value;
      select.innerHTML = areas.map((a) => `<option value="${escapeHTML(a)}">${escapeHTML(a)}</option>`).join('');
      const compativel = [...select.options].find((opt) => areaKey(opt.value) === areaKey(atual));
  ...
  async function cadastrarNovaArea(event) {
    ...
    try {
      ...
      const { error } = await sb.from('areas_cursos_v36').upsert({ nome: nova, slug, ativo: true }, { onConflict: 'slug' });
      if (error) throw error;
      if (!GC.areasDb.some((item) => areaKey(item) === areaKey(nova))) GC.areasDb.push(nova);
    } catch (error) {
      console.warn('Área salva somente neste navegador até a migration V36 ser aplicada:', error.message);
    }
    renderAreasSelects(nova);
    fecharModalNovaArea();
  }
        ...
        <td data-label="Curso" class="col-curso">
          <div class="curso-info">
            ...
            <div class="curso-textos">
              ...
              <div class="curso-sub">${(c.tipo_curso || 'PROFISSIONAL') === 'TECNICO' ? 'Técnico' : 'Profissional'} · ${fmtArea(c.categoria)}</div>
            </div>
          </div>
        </td>
        <td data-label="Área" class="col-area">${escapeHTML(fmtArea(c.categoria))}</td>
  ...
  async function salvarCurso(ev) {
    ...
    const area   = normalizarArea($('#fCursoArea')?.value || 'Tecnologia');
  ...
  async function abrirPainelModulos(id) {
    ...
    if (courseName) courseName.textContent = `${curso.titulo} · ${fmtArea(curso.categoria)}`;
  ...
  function wireCursosUI() {
    ...
    renderAreasSelects();
    carregarAreasCadastradas();

    $('#curFiltroArea')?.addEventListener('change', carregarCursosCompleto);
    $('#btnNovaArea')?.addEventListener('click', abrirModalNovaArea);
    $('#fecharArea')?.addEventListener('click', fecharModalNovaArea);
    $('#formArea')?.addEventListener('submit', cadastrarNovaArea);
    ...
    $('#tabCursos')?.addEventListener('click', (ev) => {
      ...
      if (ev.target.classList.contains('gc-prev')) return window.open(`/portaldoaluno/?curso=${id}`, '_blank');
