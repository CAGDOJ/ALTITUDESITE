...
function validarNome() {
  ...
  if (nomeEl) nomeEl.value = v;
  if (v.length < 3) {
    setErr(nomeEl, fb.nome, "Informe o nome completo.");
    return false;
  }
...
function validarCPF() {
  if (!cpfValido(cpfEl?.value || "")) {
    setErr(cpfEl, fb.cpf, "CPF inválido.");
    return false;
  }
...
function validarNasc() {
  if (!nascEl || !nascEl.value) {
    setErr(nascEl, fb.nascimento, "Informe sua data de nascimento.");
    return false;
  }
  setOK(nascEl, fb.nascimento, "");
  return true;
}

function validarEmail() {
  if (!emailValido(emailEl?.value)) {
    setErr(emailEl, fb.email, "E-mail inválido. Ex.: nome@dominio.com");
    return false;
  }
  setOK(emailEl, fb.email, "");
  return true;
}

function validarSenha() {
  if (!senhaEl?.value || senhaEl.value.length < 8) {
    setErr(senhaEl, fb.senha, "Mínimo de 8 caracteres.");
    return false;
  }
  setOK(senhaEl, fb.senha, "");
  return true;
}

function validarConf() {
  if (!confEl || confEl.value !== senhaEl.value || (confEl.value || "").length < 8) {
    setErr(confEl, fb.confirma, "Senhas não são iguais.");
    return false;
  }
  setOK(confEl, fb.confirma, "Senhas conferem.");
  return true;
}

function validarTel() {
  if (!telValido(telEl?.value || "")) {
    setErr(telEl, fb.tel, "Informe DDD + número (10 ou 11 dígitos).");
    return false;
  }
  setOK(telEl, fb.tel, "");
  return true;
...
function validarObj() {
  if (!objetivo || !objetivo.value) {
    setErr(objetivo, fb.objetivo, "Selecione uma opção.");
    return false;
  }
  setOK(objetivo, fb.objetivo, "");
  return true;
}

function isTermsChecked() {
  return termosEl ? !!termosEl.checked : true;
}
...
emailEl?.addEventListener("input", debouncedEmailCheck);
...
form?.addEventListener("submit", async (e) => {
  ...
  if (cpfUsed || emailUsed) {
    showFormMsg("error", "Existem campos com erro. Corrija-os para continuar.");
    toggleSubmit();
    return;
  }
  ...
  const payloadAluno = {
    ...
    cpf: cpfRaw,
    data_nascimento: nascEl?.value || null,
