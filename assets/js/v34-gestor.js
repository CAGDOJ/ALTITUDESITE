(() => {
  ...
  function renderCommercial(){
    ...
    const p=$('listaPromocoesGestao');if(p)p.innerHTML=state.promocoes.length?state.promocoes.map(x=>`<article class="commercial-card media-card">${x.imagem_url?`<img src="${esc(x.imagem_url)}" alt="">`:''}<div><span>/${esc(x.slug)}</span><h4>${esc(x.titulo)}</h4><p>${esc(x.descricao||'')}</p><small>${x.ativa?'Visível':'Oculto'} · ${esc(x.frequencia)}</small></div><div><button data-edit-promo="${x.id}">Editar</button><button data-toggle-promo="${x.id}">${x.ativa?'Pausar':'Ativar'}</button><a href="/portaldoaluno/?promocao=${encodeURIComponent(x.slug)}" target="_blank">Abrir endereço</a><button class="danger" data-delete-promo="${x.id}">Excluir</button></div></article>`).join(''):'<p class="empty-state">Nenhum anúncio cadastrado.</p>';
