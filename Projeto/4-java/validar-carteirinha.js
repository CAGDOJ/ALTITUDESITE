(async () => {
  ...
  try {
    ...
    const card = data[0];
    loading.hidden = true; result.hidden = false;
    result.innerHTML = `<div class="validation-state ok">✓</div><h1>Carteirinha válida</h1><p class="validation-sub">Vínculo confirmado na base oficial do ALTITUDE CENTRO UNIVERSITÁRIO.</p><dl><div><dt>Aluno</dt><dd>${escapeHtml(card.nome)}</dd></div><div><dt>Registro acadêmico</dt><dd>${escapeHtml(card.ra || '—')}</dd></div><div><dt>Status</dt><dd>${escapeHtml(card.status)}</dd></div><div><dt>Cursos vinculados</dt><dd>${Number(card.cursos_ativos || 0)}</dd></div></dl>`;
